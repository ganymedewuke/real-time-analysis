...
public class ArealDistributionProcessData {
	...
	public static void main(String[] args) throws Exception {
		...
		DataStream<ArealDistribution> map = input.flatMap(new ArealDistributionMap());


		DataStream<ArealDistribution> reduce = map.keyBy("groupByField").
				countWindow(Long.valueOf(parameterTool.getRequired("windows.size"))).
				reduce(new ArealDistributionReduce());

		//打印reduce的内容
//		reduce.print();
		reduce.addSink(new ArealDistributionSinkReduce()).name("ArealDistributionReduce");

		env.execute("ArealDistributionProcessData");
	}
}
